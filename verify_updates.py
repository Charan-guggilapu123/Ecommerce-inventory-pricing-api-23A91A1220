...
def test_seasonal_pricing():
    ...
    print("✅ Seasonal Pricing Correct")
...
def test_checkout_flow():
    ...
    print(f"Initial Stock: {inv.stock_quantity}, Reserved: {inv.reserved_quantity}")
    ...
    inv.refresh_from_db()
    print(f"After Add (2 units): Stock: {inv.stock_quantity}, Reserved: {inv.reserved_quantity}")
    ...
    inv.refresh_from_db()
    print(f"After Checkout: Stock: {inv.stock_quantity}, Reserved: {inv.reserved_quantity}")
    
    assert inv.stock_quantity == 8
    assert inv.reserved_quantity == 0
    assert not Cart.objects.filter(id=cart.id).exists()
    print("✅ Checkout Successful (Stock deducted, cart cleared)")
    
    # Cleanup
    cat.delete() # cascade should delete others
